...
@pytest.mark.dataframe
def test_dataframe_single_date():
    ts_codes = [12, 433]
    df = dataframe(ts_codes, start="02/01/2018")
    assert df.shape == (2, 2)

@pytest.mark.dataframe
def test_dataframe_single_date_with_strict_as_true():
    ts_codes = [12, 433]
    df = dataframe(ts_codes, start="02/01/2018", strict=True)
    assert df.shape == (1, 2)
